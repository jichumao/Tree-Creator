...
static PRM_Name	grammarFileName("grammarFile", "Grammarfile");
...
static PRM_Default angleDefault(10.0);
static PRM_Default stepSizeDefault(5.0);
...
SOP_Lsystem::cookMySop(OP_Context &context)
{
	...
	float iter = ITERATIONS(now);

	UT_String gFileUTStr = "";
	GRAMMAR(gFileUTStr, now);
	std::string gFileStdStr = gFileUTStr.toStdString();
	...
	myplant.loadProgram(gFileStdStr);
