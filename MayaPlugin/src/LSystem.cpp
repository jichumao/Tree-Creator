...
void LSystem::processPy(unsigned int n,
    ...
    std::vector<float> postFlower;
    for (int i = 0; i < preFlowers.size(); i++)
    {
        vec3 pos = preFlowers[i].first;
        postFlower.push_back(pos[0]);
        postFlower.push_back(pos[1]);
        postFlower.push_back(pos[2]);

        flowers.push_back(postFlower);
        postFlower.clear();
    }

    std::vector<float> postBranch;
    for (unsigned int i = 0; i < preBranches.size(); i++) {
        vec3 start = preBranches[i].first;
        vec3 end = preBranches[i].second;

        postBranch.push_back(start[0]);
        postBranch.push_back(start[1]);
        postBranch.push_back(start[2]);
        postBranch.push_back(end[0]);
        postBranch.push_back(end[1]);
        postBranch.push_back(end[2]);

        branches.push_back(postBranch);
        postBranch.clear();
    }
}
